...
class FEM
{
 public:
  ...
  double c0 = 1./3.; // 1/m
  double z_min = 0., z_max = 0.02; // m
  double y_min = 0., y_max = 0.08; // m
  double x_min = 0., x_max = 0.04; // m
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int node = 0; node < totalNodes; node++){
    ...
    if( nodeLocation[node][0] == x_min ){
      boundary_values[node] = 300.*(1+c0*(nodeLocation[node][1]+nodeLocation[node][2]));
    }
    // Right BC
    if( nodeLocation[node][0] == x_max ){
      boundary_values[node] = 310.*(1+c0*(nodeLocation[node][1]+nodeLocation[node][2]));
    }
  }
}
